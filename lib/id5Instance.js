...
export class Id5Instance {
  ...
  bootstrap() {
    ...
    this._multiplexingInstance
      .on(ApiEvent.CASCADE_NEEDED, cascadeCommand => {
        const options = this.config.getOptions();
        if (cascadeCommand.partnerId === options.partnerId && options.maxCascades >= 0 && !this.config.hasCreativeRestrictions()) {
          const isSync = options.partnerUserId && options.partnerUserId.length > 0;
          const gppPart = cascadeCommand.gppString ? `&gpp_string=${cascadeCommand.gppString}:&gpp_sid=${cascadeCommand.gppSid}` : '';
          ...
          this._log.info('Opportunities to cascade available', syncUrl);
    ...
    this._gatherFetchIdData().then(data => {
    ...
    this._submitRefreshConsent().then(consentData => {
      if (consentData) {
  ...
  async _gatherFetchIdData() {
    const options = this.config.getOptions();
    const uaHints = await gatherUaHints(options.disableUaHints, this._log);
    return {
