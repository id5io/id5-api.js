...
import {convertPartnerDataToPd} from './partnerDataConfig.js';
...
export const API_STANDARD_ORIGIN = 'api';
...
export class Id5Instance extends CoreId5Instance {
   ...
   */
  getGpId() {
    return this._isExposed === false ? undefined : this._gpId;
  }
   ...
   */
  getUserIdsAsEids() {
    return this._ids === undefined ? [this.getUserIdAsEid()] : Object.values(this._ids).map(value => value.eid);
  }
   ...
   */
  constructor(config, clientStore, consentManagement, metrics, consentDataProvider, logger, multiplexingInstance, pageLevelInfo, trueLinkAdapter) {
    super(config, metrics, logger, multiplexingInstance, pageLevelInfo, API_STANDARD_ORIGIN, {
      canDoCascade: !config.hasCreativeRestrictions() && config.getOptions().idLookupMode !== true
    });
  ...
  async _gatherFetchIdData() {
    ...
    const commonData = await super._gatherFetchIdData();

    // Convert partnerData to pd if present (takes precedence over pd)
    let pd = options.pd;
    if (options.partnerData) {
      const convertedPd = await convertPartnerDataToPd(options.partnerData, this._log);
      if (convertedPd !== undefined) {
        pd = convertedPd;
      }
    }
  ...
  /**
   * @returns {string} a most recent id5 signature
   */
  getSignature(){
