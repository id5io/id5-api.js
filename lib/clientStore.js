...
export default class ClientStore {
  ...
  getHashedSegments(partnerId) {
    return this.get(ClientStore.segmentsCacheConfig(partnerId));
  }
  ...
  putHashedSegments(partnerId, segments) {
    this.put(ClientStore.segmentsCacheConfig(partnerId), ClientStore.makeStoredHash(JSON.stringify(segments)));
  }
  ...
  clearHashedSegments(partnerId) {
    this.clear(ClientStore.segmentsCacheConfig(partnerId));
  }

  /**
   * Generate local storage config for PD of a given partner
   * @param {number} partnerId
   * @return {{expiresDays: number, name: string}}
   */
   ...
   * @return {{expiresDays: number, name: string}}
   */
  static segmentsCacheConfig(partnerId) {
    return {
      name: `${CONSTANTS.STORAGE_CONFIG.SEGMENTS.name}_${partnerId}`,
      expiresDays: CONSTANTS.STORAGE_CONFIG.SEGMENTS.expiresDays
    };
  }
