...
export default class ClientStore {
  ...
  get(cacheConfig) {
    try {
      const localStorageGrant = this.localStorageGrantChecker();
      if (localStorageGrant.isDefinitivelyAllowed()) {
        return this.localStorage.getItemWithExpiration(cacheConfig);
      } else {
        logWarn(this.invocationId, 'clientStore.get() has been called without without definitive grant', localStorageGrant);
      }
    } catch (e) {
