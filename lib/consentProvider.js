...
export class ConsentDataProvider {
  ...
  _parseStaticConsentData(data, finalCallback) {
    ...
    if (isPlainObject(data.getTCData)) {
      mergeData = this._parseTcfData(data.getTCData);
      try {
        const metrics = this._metrics;
        this._lookupTcf((foundTcf) => {
          metrics.consentDiscrepancyCounter({
            apiType: API_TYPE.TCF_V2,
            sameString: foundTcf?.consentString === mergeData?.consentString,
            sameLSPC: foundTcf?.localStoragePurposeConsent === mergeData?.localStoragePurposeConsent,
            sameGdpr: foundTcf?.gdprApplies === mergeData?.gdprApplies,
            sameApi: foundTcf?.api === mergeData?.api
          }).inc();
        });
      } catch (e) {
        // do nothing
      ...
      try {
        ...
        this._lookupUsp((foundUsp) => {
          metrics.consentDiscrepancyCounter({
            ...
            sameString: foundUsp?.ccpaString === mergeData?.ccpaString,
            sameApi: foundUsp?.api === mergeData?.api
          }).inc();
        });
      } catch (e) {
        // do nothing
    ...
    try {
      ...
      if (client) {
        ...
        try {
          ...
          let consentDataPart = {
            api: consentData.version,
            gppData: consentData
          };
          callback(consentDataPart);
  ...
  _lookupDirectTcf(cmpFunction, callback) {
    ...
    const tcfResponseCallback = (tcfData, success) => {
      ...
      if (tcfData &&
        ...
        callback(this._parseTcfData(tcfData));
      } else {
        return false; // to not deregister the message listener in case of indirect communication
      }
    };
