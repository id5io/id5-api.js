...
import {API_TYPE, ConsentData, NoopLogger} from '@id5io/multiplexing';
...
export class ConsentDataProvider {
   ...
   * @param {Logger} log
   */
  constructor(metrics, log = NoopLogger) {
  ...
  _lookupIabConsent(finalCallback) {
    ...
    this._lookupGpp(consentData, done);
  ...
  _lookupGpp(consentData, done) {
    ...
    let applicableEvents = event => event?.eventName === 'cmpStatus' || event?.eventName === 'sectionChange';

    if (isFn(gppApiFunction)) {
      direct = true;
      this._log.info('cmpApi: Detected GPP is directly accessible, calling it now.');
      gppFn = gppApiFunction;
    } else {
      this._log.info('cmpApi: Detected GPP is outside the current iframe. Using message passing.');
      const surrogate = ConsentDataProvider._buildCmpSurrogate('gppv1', gppApiFrame, event => applicableEvents(event)
      );
      gppFn = function (command, callback) {
        surrogate(command, null, callback);
      };
    }

    const gppCallback = (event) => {
      let callbackDate = Date.now();
      let parsedData = ConsentDataProvider._parseGppData(event.pingData);
      let commonTags = {gppEventName: event.eventName, gppVersion: parsedData.version, directCmp: direct};
      if (applicableEvents(event)) {
        this._metrics.timer('gpp.delay', Object.assign({}, commonTags, parsedData)).record(callbackDate - lookupStart);
        let tcfDone = done[CALLBACK_POSITIONS.TCF];
        if ((tcfDone && (consentData.api === API_TYPE.TCF_V1 || consentData.api === API_TYPE.TCF_V2)) || !tcfDone) {
          let diff = tcfDone ? callbackDate - tcfDone : 0;
          this._metrics.timer('gpp.otherCmp.delay', Object.assign({}, commonTags, {apiType: consentData.api})).record(diff);
        }
        let uspDone = done[CALLBACK_POSITIONS.USP];
        if ((uspDone && consentData.api === API_TYPE.USP_V1) || !uspDone) {
          let diff = uspDone ? callbackDate - uspDone : 0;
          this._metrics.timer('gpp.otherCmp.delay', Object.assign({}, commonTags, {apiType: consentData.api})).record(diff);
        }
      } else {
        this._metrics.counter('gpp.events', commonTags).inc(1);
        this._log.error('cmpApi: GPP callback not successful. Using defaults.');
      }
    };
    gppFn('addEventListener', gppCallback);
  }
   ...
   * @param {function} deregisterPredicate a function which will be called with the callback return value,
   * and should return true if event handler should be deregistered, or false if not
   * @returns {function} the function to call
   * @private
   */
  static _buildCmpSurrogate(typeOfCall, apiFrame, deregisterPredicate = arg => true) {
    return (param0, param1, messageCallback) => {
      ...
      const eventHandler = (event) => {
        ...
        if (result && result.callId === callId) {
          ...
          if (deregisterPredicate(result.returnValue)) {
  ...
  static _parseGppData(pingData) {
    return {
      version: pingData?.gppVersion,
      cmpStatus: pingData?.cmpStatus,
      supportedAPIs: pingData?.supportedAPIs,
