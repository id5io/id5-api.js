  ...
  _lookupGpp(consentData, done) {
    ...
    const gppCallback = (event) => {
      ...
      if (applicableEvents(event)) {
        ...
        if ((done[0] > 0 && (consentData.api === API_TYPE.TCF_V1 || consentData.api === API_TYPE.TCF_V2)) || done[0] === 0) {
          let diff = done[0] > 0 ? callbackDate - done[0] : 0;
          this._metrics.timer('gpp.otherCmp.delay', Object.assign({}, commonTags, {api: consentData.api})).record(diff);
        }
        if ((done[1] > 0 && consentData.api === API_TYPE.USP_V1) || done[1] === 0) {
          let diff = done[1] > 0 ? callbackDate - done[1] : 0;
          this._metrics.timer('gpp.otherCmp.delay', Object.assign({}, commonTags, {api: consentData.api})).record(diff);
        }
      } else {
        this._metrics.counter('gpp.events', commonTags);
