...
export default class Id5Status {
   ...
   */
  constructor(invocationId, config, clientStore, consentManagement, liveIntentApi) {
  ...
  setUserId(response, fromCache) {
    ...
    const ext = response.ext || {'linkType': 0};
    ...
    const hasChanged = this._userId !== userId || this._ext.linkType !== ext.linkType;
   ...
   */
  getLinkType() {
    return this._isExposed === false ? 0 : this._ext ? this._ext.linkType : undefined;
  }
   ...
   */
  getExt() {
    let exposedExt = this._isExposed === false ? {linkType: 0} : this._ext;
