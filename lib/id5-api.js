...
class Id5Api {
  ...
  init(passedOptions) {
    ...
    this.invocationId += 1;
    try {
      logInfo(invocationId, `ID5 API version ${this._version}. Invoking init()`, arguments);
  ...
  getId(id5Status, forceFetch = false) {
    ...
    id5Status.consentManagement.requestConsent(options.debugBypassConsent, options.cmpApi, options.consentData, (consentData) => {
      ...
      if (
        ...
        if (forceFetch) {
          logInfo(id5Status.invocationId, '...with Force Fetch');
        }
        ajax(url, {
          success: response => {
            logInfo(id5Status.invocationId, 'Response from ID5 received:', response);
            let responseObj;
            if (response) {
              try {
                ...
                if (isStr(responseObj.universal_uid)) {
                  ...
                  logError(id5Status.invocationId, 'Invalid response from ID5 servers:', response);
                }
              } catch (error) {
                logError(id5Status.invocationId, error);
              }
            } else {
