...
} from '@id5io/diagnostics';
import {Id5Integration} from '@id5io/multiplexing';
...
class Id5Api {
  ...
  init(passedOptions) {
    ...
    try {
      ...
      const metrics = this._configureDiagnostics(options);
      if (metrics) {
      ...
      const logger = new InvocationLogger(this.invocationId);
      const integrationInstance = this._registerIntegration(config, metrics, logger);
      ...
      const partnerStatus = new Id5Status(invocationId, config, clientStore, consentManagement, liveIntentApi, metrics);
      ...
      // at this development stage just for testing purpose, to get access
      partnerStatus._integrationInstance = integrationInstance;
  ...
  getIdWithUaHints(id5Status, uaHints, forceFetch) {
    ...
    let consentRequestTimeMeasurement = id5Status._metrics?.consentRequestTimer(options.debugBypassConsent ? 'bypass' : options.cmpApi).startMeasurement();
    id5Status.consentManagement.requestConsent(options.debugBypassConsent, options.cmpApi, options.consentData, (consentData) => {
      if (consentRequestTimeMeasurement) {
        consentRequestTimeMeasurement.record();
      }
      const localStorageGrant = id5Status.localStorageGrant();
      ...
      if (
        ...
        let extensionsCallTimeMeasurement = id5Status._metrics?.extensionsCallTimer().startMeasurement();
  ...
  fetchFreshID5ID(data, options, consentData, id5Status, forceFetch, cachedResponseUsed) {
    ...
    ajax(url, {
      success: this.handleSuccessfulFetchResponse(id5Status, options, cachedResponseUsed, consentData, fetchTimeMeasurement),
      error: error => {
        logError(id5Status.invocationId, 'Error during AJAX request to ID5 server', error);
        if (fetchTimeMeasurement) {
          fetchTimeMeasurement.record(id5Status._metrics?.fetchFailureCallTimer());
        }
      }
    }, JSON.stringify(data), {method: 'POST', withCredentials: true});
  }

  handleSuccessfulFetchResponse(id5Status, options, cachedResponseUsed, consentData, fetchTimeMeasurement) {
    return response => {
      if (fetchTimeMeasurement) {
        fetchTimeMeasurement.record(id5Status._metrics?.fetchSuccessfulCallTimer());
      }
  ...
  _configureDiagnostics(options) {
    try {
      ...
      logError(this.invocationId, 'Failed to configure diagnostics', e);
      return undefined;
    }
  }
   ...
   * @param {Logger} logger
   * @return {Id5Integration.Instance}
   * @private
   */
  _registerIntegration(config, metrics, logger) {
    try {
      const instance = new Id5Integration.Instance(window, ORIGIN, currentVersion, config, metrics, logger);
      instance.register();
      return instance;
    } catch (e) {
      logError('Failed to register integration instance', e);
    }
  }
}
