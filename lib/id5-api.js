...
class Id5Api {
  ...
  constructor() {
    ...
    this._metrics = new Id5CommonMetrics(new MeterRegistry());
  }
  ...
  init(passedOptions) {
    ...
    try {
      ...
      this._configureDiagnostics(options);
      this._metrics.loadDelay().recordNow(); // records time elapsed since page visit
      this._metrics.invocationCount().inc();
  ...
  getIdWithUaHints(id5Status, uaHints, forceFetch) {
    ...
    let consentRequestTimeMeasurement = this._metrics.consentRequestTime(options.debugBypassConsent ? 'bypass' : options.cmpApi).startMeasurement();
    id5Status.consentManagement.requestConsent(options.debugBypassConsent, options.cmpApi, options.consentData, (consentData) => {
      consentRequestTimeMeasurement.stop();
      const localStorageGrant = id5Status.localStorageGrant();
      ...
      if (
        ...
        let extensionsCallTimeMeasurement = this._metrics.extensionsCallTime().startMeasurement();
        ...
        EXTENSIONS.gather(id5Status.invocationId)
          .then(extensions => {
            extensionsCallTimeMeasurement.stop();
  ...
  fetchFreshID5ID(data, options, consentData, id5Status, forceFetch, cachedResponseUsed) {
    ...
    let fetchTimeMeasurement = this._metrics.fetchCallTime().startMeasurement();
    ...
    ajax(url, {
      ...
      error: error => {
        ...
        fetchTimeMeasurement.stop();
        id5This._metrics.fetchErrorCount().inc();
      }
  ...
  _configureDiagnostics(options) {
    let commonTags = new Id5CommonTags('api', currentVersion, options.partnerId);
    this._metrics.registry.addCommonTags({
      ...commonTags,
      tml: this._referer.topmostLocation,
      ref: this._referer.ref,
      curl: this._referer.canonicalUrl
    });
    if (!options.diagnostics?.publishingDisabled) {
      let publisher = createPublisher(0.1);
      if (options.diagnostics?.publishInMsec) {
        this._metrics.registry.schedulePublishAfterMsec(options.diagnostics?.publishInMsec, publisher);
      }
      if (options.diagnostics?.publishOnUnload) {
        this._metrics.registry.schedulePublishOnUnload(publisher);
      }
    }
  }
}
