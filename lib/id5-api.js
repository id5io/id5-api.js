...
class Id5Api {
  ...
  constructor() {
    ...
    try {
      this._metrics = new Id5CommonMetrics('api', currentVersion);
    } catch (e) {
      logError(this.invocationId, 'Failed to create metrics', e);
    }
  }
  ...
  init(passedOptions) {
    ...
    try {
      ...
      if (this._metrics) {
        this._metrics.loadDelayTimer().recordNow(); // records time elapsed since page visit
        this._metrics.invocationCounter().inc();
      }
  ...
  getIdWithUaHints(id5Status, uaHints, forceFetch) {
    ...
    let consentRequestTimeMeasurement = this._metrics?.consentRequestTimer(options.debugBypassConsent ? 'bypass' : options.cmpApi).startMeasurement();
    id5Status.consentManagement.requestConsent(options.debugBypassConsent, options.cmpApi, options.consentData, (consentData) => {
      ...
      if (
        ...
        let extensionsCallTimeMeasurement = this._metrics?.extensionsCallTimer().startMeasurement();
    ...
    ajax(url, {
      ...
      error: error => {
        ...
        if (fetchTimeMeasurement) {
          fetchTimeMeasurement.record(id5This._metrics?.fetchFailureCallTimer());
        }
      }
  ...
  handleSuccessfulFetchResponse(id5Status, options, cachedResponseUsed, consentData, fetchTimeMeasurement) {
    ...
    return response => {
      if (fetchTimeMeasurement) {
        fetchTimeMeasurement.record(id5This._metrics?.fetchSuccessfulCallTimer());
      }
      ...
      try {
        ...
        if (!options.diagnostics?.publishingDisabled) {
          ...
          if (options.diagnostics?.publishAfterLoadInMsec && options.diagnostics.publishAfterLoadInMsec > 0) {
            this._metrics.schedulePublishAfterMsec(options.diagnostics.publishAfterLoadInMsec, publisher);
          }
          if (options.diagnostics?.publishBeforeWindowUnload) {
            this._metrics.schedulePublishBeforeUnload(publisher);
          }
        }
      } catch (e) {
