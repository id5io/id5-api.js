...
const LB_HOST = 'https://lb.eu-1-id5-sync.com';
...
class Id5Api {
  ...
  getId(id5Status, forceFetch = false) {
    const uaData = delve(window, 'navigator.userAgentData');
    if (isDefined(uaData) && !id5Status.getOptions().disableUaHints) {
      uaData.getHighEntropyValues(['architecture', 'fullVersionList', 'model', 'platformVersion'])
  ...
  getIdWithUaHints(id5Status, uaHints, forceFetch) {
    ...
    id5Status.consentManagement.requestConsent(options.debugBypassConsent, options.cmpApi, options.consentData, (consentData) => {
      ...
      if (
        ...
        const data = this.gatherData(options, id5Status, nb, consentData, storedResponse, uaHints);
        this.gatherLbData(lbData => {
          data.extensions = {
            lb: lbData,
  ...
  gatherData(options, id5Status, nb, consentData, storedResponse, uaHints) {
    const data = {
    ...
    return response => {
      ...
      try {
        ...
        if (responseObj.cascade_needed === true && localStorageGrant.isDefinitivelyAllowed() && options.maxCascades >= 0 && !options.applyCreativeRestrictions) {
          ...
          // TODO this is the same logic as before, though it leads to many calls to sync with "undefined" gdpr_consent
          const gdprConsentString = consentData.gdprApplies ? consentData.consentString : undefined;
          const syncUrl = `${HOST}/${isSync ? 's' : 'i'}/${options.partnerId}/${options.maxCascades}.gif?id5id=${id5Status._userId}&o=api&${isSync ? 'puid=' + options.partnerUserId + '&' : ''}gdpr_consent=${gdprConsentString}&gdpr=${consentData.gdprApplies}`;
  ...
  gatherLbData(successCallback, id5Status) {
    const lbUrl = `${LB_HOST}/lb/v1`;

    ajax(lbUrl, {
      success: response => {
        const jsonResponse = this.validateResponseIsCorrectJson(response, id5Status, 'lb');
        successCallback(jsonResponse ? jsonResponse.lb : null);
      },
      error: error => {
        logWarn(id5Status.invocationId, 'Got error from lb endpoint', error);
