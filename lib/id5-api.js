...
class Id5Api {
  ...
  init(options) {
    ...
    try {
      ...
      // This is a quick workaround. Correct usage of option applyCreativeRestrictions should be local, not global
              ...
              if (response) {
                try {
                  ...
                  if (isStr(responseObj.universal_uid)) {
                    ...
                    if (responseObj.cascade_needed === true && this.localStorageAllowed === true && options.maxCascades >= 0 && !options.applyCreativeRestrictions) {
