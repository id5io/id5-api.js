...
import multiplexing, {ApiEvent} from '@id5io/multiplexing';
...
class Id5Api {
  ...
  init(passedOptions) {
    ...
    try {
      ...
      const partnerStatus = new Id5Status(invocationId, config, clientStore, consentManagement, liveIntentApi, metrics, consentDataProvider, log, instance);
      instance
        .on(ApiEvent.CASCADE_NEEDED, this._doCascade(partnerStatus))
        .on(ApiEvent.USER_ID_READY, userIdData => partnerStatus.setUserId(userIdData.responseObj, userIdData.isFromCache))
        .on(ApiEvent.USER_ID_FETCH_CANCELED, details => log.info('ID5 User ID fetch canceled:', details.reason));
      this._gatherFetchIdData(partnerStatus).then(data => instance.register(data, {options: partnerStatus.getOptions()}));
  ...
  _submitRefreshConsent(options, consentDataProvider, instance, metrics, log) {
    ...
    consentDataProvider.refreshConsentData(options.debugBypassConsent, options.cmpApi, options.consentData)
      .then(consentData => {
        consentRequestTimeMeasurement.record(metrics.consentRequestTimer(consentRequestType, {success: true}));
  ...
  refreshId(id5Status, forceFetch = false, options = {}) {
    ...
    try {
      ...
      this._gatherFetchIdData(id5Status)
        .then(fetchIdData => id5Status.instance.refresh(fetchIdData, {
          resetConsent: true,
          forceAllowLocalStorageGrant: updatedOptions.allowLocalStorageWithoutConsentApi || updatedOptions.debugBypassConsent,
          forceFetch: forceFetch
        }));
      this._submitRefreshConsent(updatedOptions, id5Status._consentDataProvider, id5Status.instance, id5Status._metrics, id5Status._logger);
    } catch (e) {
  ...
  _gatherFetchIdData(id5Status) {
    ...
    return this.gatherUaHints(options)
      ...
      .catch(error => logError('Error while calling navigator.userAgentData.getHighEntropyValues()', error))
      .then(uaHints => {
        return {
          ...
          refreshInSeconds: options.refreshInSeconds,
