...
import {invocationCountSummary, loadDelayTimer, Id5CommonMetrics, partnerTag, pbjsDetectedCounter} from './metrics.js';
import {Id5Api} from './core/id5Api.js';
import enableEventsTracking from './prebid/eventsTracker.js';
import {PbjsDetector} from './prebid/pbjsUtils.js';
import {PARTNER_DATA_KEYS} from './partnerDataConfig.js';
...
export class ApiStandard {
  ...
  init(passedOptions) {
    ...
    try {
      ...
      setTimeout(() => {
        enableEventsTracking(config.getOptions().partnerId, log);
        checkPbjsId5Integrations(metrics, log);
      }, 0);
      return id5Instance;
    } catch (e) {
  ...
  _configureDiagnostics(options, log) {
    try {
      ...
      metrics.addCommonTags({
        ...
        idLookupMode: options.idLookupMode === true ? 'true' : 'false'
      });
...
function checkPbjsId5Integrations(metrics, log) {
  new PbjsDetector(window, log).getPbjs().then(
    pbjs => {
      if (metrics && pbjs) {
        const installedModules = Array.isArray(pbjs.installedModules) ? pbjs.installedModules : [];
        const hasId5IdModule = installedModules.includes('id5IdSystem');
        const hasId5AnalyticsModule = installedModules.includes('id5AnalyticsAdapter');

        pbjsDetectedCounter(metrics, {
          hasId5IdModule,
          hasId5AnalyticsModule
        }).inc();
      }
    });
}
