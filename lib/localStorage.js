...
export default class LocalStorage {
   ...
   * @returns {string|null|undefined} the stored value, null if no value or expired,
   * undefined if no localStorage
   */
  getItemWithExpiration({name}) {
    if (this.available) {
      const storedValueExp = this.win.localStorage.getItem(name + EXP_SUFFIX);
  ...
  setItemWithExpiration({name, expiresDays}, value) {
    ...
    this.storeItem(name + EXP_SUFFIX, expiresStr);
    this.storeItem(name, value);
  }
