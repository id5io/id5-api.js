...
import {PbjsDetector} from './pbjsUtils.js';
...
/**
* @typedef {import('./pbjsUtils.js').PrebidGlobal} PrebidGlobal
*/
...
const STANDARD_EVENTS_TO_TRACK = [
  ...
  PREBID_EVENTS.AD_RENDER_FAILED,
  PREBID_EVENTS.AD_RENDER_SUCCEEDED
];
...
EVENT_PAYLOAD_BUILDERS[PREBID_EVENTS.AD_RENDER_FAILED] = copyAdRenderFailed;
EVENT_PAYLOAD_BUILDERS[PREBID_EVENTS.AD_RENDER_SUCCEEDED] = copyAdRenderSucceeded;
...
class EventsTracker {
  ...
  track(eventType, payload) {
    ...
    try {
      ...
      return this.sendEvent(this.makeEvent(eventType, payload));
    } catch (error) {
      this._logger.error('pbjsEventsTracker: ERROR', error);
      return this.sendErrorEvent(error);
    }
  }

  async sendEvent(eventToSend) {
    const jsonString = JSON.stringify(eventToSend);
    const shouldCompress = jsonString.length > 1024 && 'CompressionStream' in window;
    ...
    try {
      ...
      if (shouldCompress) {
        body = await new Response(
          new Response(jsonString).body.pipeThrough(new CompressionStream('gzip'))
        ).blob();

        headers['Content-Encoding'] = 'gzip';
      }

      return fetch(this._ingestUrl, {
        method: 'POST',
        headers: headers,
        body: body
      }).catch((e) => {
  ...
  return fetch(`${CONFIG_URL_PREFIX}/${partnerId}/${SOURCE}`)
    ...
    .then((configFromServer) => {
      ...
      if (sampling > 0 && Math.random() < (1 / sampling)) {
        ...
        if (isFn(prebidGlobal.getEvents) && isArray(prebidGlobal.getEvents())) {
          prebidGlobal.getEvents().forEach((event) => {
            ...
            if (event && eventsToTrack.indexOf(event.eventType) >= 0) {
              ...
              trackPromises.push(eventsTracker.track(event.eventType, event.args));
            } else {
        ...
        return Promise.all(trackPromises).then(() => true)
      } else {
        return false;
      }
    })
...
function copyAdRenderSucceeded(payload) {
  return {adId: payload.adId, bid: copyBidWon(payload.bid)}
}

function copyAdRenderFailed(payload) {
  return {adId: payload.adId, reason: payload.reason}
}
...
export default function (partnerId, logger = NO_OP_LOGGER) {
  try {
    ...
    if (window.id5_pbjs_et[partnerId] !== true) {
      ...
      new PbjsDetector(window, logger).getPbjs().then((pbjs) => {
        registerEventsTracker(pbjs, partnerId, logger);
      });
    }
  } catch (e) {
