...
import { delve, isFn, isPlainObject, isStr, logError, logInfo } from './utils.js';
...
export class LiveIntentApi {
  ...
  constructor(windowObj, isEnabled, invocationId, localStorage, storageConfig) {
    const self = this;
  ...
  _onDetected() {
    ...
    if (isFn(resolve)) {
      try {
        ...
        logError('Error caught while calling resolve() on LiveIntent API', e);
      }
    }
  }
