 ...
 * @property {PartnerData} [partnerData] - Partner data with semantic keys for cross-domain reconciliation.
 * @see Readme.md
 ...
 * @property {string} [other] - Other data
 * @property {string} [hem] - Hashed email (auto-normalized and SHA256 hashed if not already hashed)
 * @property {string} [phone] - Hashed phone number (trimmed and SHA256 hashed if not already hashed)
 * @property {string} [xpuid] - Cross-partner user ID value
 * @property {string} [xpuidSource] - Cross-partner user ID source
 * @property {string} [puid] - Partner-specific user ID value
 * @property {string} [idfa] - Apple IDFA (automatically lowercased)
 * @property {string} [gaid] - Google Advertising ID (automatically lowercased)
 * @property {string} [url] - Page URL
 * @property {string} [domain] - Page domain
 * @property {string} [ipv4] - IPv4 address
 * @property {string} [ipv6] - IPv6 address
 * @property {string} [ua] - User agent
 * @property {string} [isBurnerEmail] - Is burner email
 * @property {string} [idfv] - Apple IDFV (automatically lowercased)
 * @property {string} [iabToken] - IAB token
 */
...
export class Config {
  ...
  updOptions(providedOptions) {
    ...
    // Process partnerData first if present
    if (providedOptions.partnerData !== undefined) {
      const partnerData = providedOptions.partnerData;

      // If partnerData is provided, it takes precedence over pd
      if (isDefined(providedOptions.pd)) {
        log.info('Both pd and partnerData provided. Using partnerData and ignoring pd.');
      }

      // Store partnerData for later conversion (in _gatherFetchIdData)
      // Don't convert here since conversion is async and we need to keep updOptions sync
      this.options.partnerData = partnerData;
      this.providedOptions.partnerData = partnerData;
    }
    ...
    Object.keys(providedOptions).forEach(topic => {
      ...
      } else if (topic !== 'partnerId' && topic !== 'partnerData') { // Already dealt with
