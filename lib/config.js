 ...
 */
import { logError, isA, isArray, isStr, all } from './utils';
 ...
 * @property {string} [seat] - Seat ID at destination. Optional.
    ...
    Object.keys(options).forEach(topic => {
      if (topic === 'segments') {
        ...
        if (!isArray(segments)) {
          ...
          self.options[topic] = [];
          self.providedOptions[topic] = [];
          segments.forEach((segment, index) => {
            if (!isArray(segment['ids']) || !all(segment['ids'], isStr)) {
              logTypeError(`segments[${index}].ids`, 'Array of String', segment['ids']);
              self.invalidSegments += 1;
              return;
            }
            if (segment['ids'].length < 1) {
              logError(`Config option segments[${index}].ids should contain at least one segment ID`);
              self.invalidSegments += 1;
              return;
            }
            if (!isStr(segment['destination'])) {
              logTypeError(`segments[${index}].destination`, 'Sring', segment['destination']);
              self.invalidSegments += 1;
              return;
            }
            self.options[topic].push({
              ids: segment['ids'],
              destination: segment['destination']
            });
            self.providedOptions[topic].push({
              ids: segment['ids'],
              destination: segment['destination']
            });
          }, segments);
