...
export class ConsentData {
  ...
  canIUseLocalStorage() {
    switch (this.api) {
      ...
      case API_TYPE.TCF_V1:
        return !this.gdprApplies || delve(this, 'vendorData.purposeConsents.1') !== false;
      case API_TYPE.TCF_V2:
        return !this.gdprApplies || delve(this, 'vendorData.purpose.consents.1') !== false;
   ...
   */
  hashCode() {
    // excluding the vendorConsents object since the consentString
    // is enough to know. CCPA string doesn't pose the need to compare
    // since doesn't contribute to the local storage decision.
    return cyrb53Hash(JSON.stringify({
  ...
  parseStaticConsentData(data, finalCallback) {
    ...
    if (isPlainObject(data.getConsentData)) {
      ...
      mergeData = {
        ...
        allowedVendors: data.allowedVendors.map(item => '' + item),
        gdprApplies: true
      };
    } else if (isPlainObject(data.getUSPData)) {
      mergeData = ConsentManagement.parseUspData(data.getUSPData);
    } else {
      logWarn('cmpApi: No static consent data detected! Using defaults.');
    }
    logInfo(`cmpApi: Detected API '${this.consentData.api}' from static consent data`, data);
