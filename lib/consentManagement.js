...
export class ConsentManagement {
  ...
  isLocalStorageAllowed(allowLocalStorageWithoutConsentApi, debugBypassConsent) {
    const consentData = this.consentData;
    ...
    if (consentData.apiVersion === 1) {
      return delve(consentData, 'vendorData.purposeConsents.1') !== false;
    }

    if (consentData.apiVersion === 2) {
      return delve(consentData, 'vendorData.purpose.consents.1') !== false;
    }
