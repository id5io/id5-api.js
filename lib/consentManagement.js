...
export class ConsentManagement {
  ...
  /**
   * Used to ask for consent only once per instance
   * @type {boolean}
   */
  ...
  requestConsent(debugBypassConsent, cmpApi, providedConsentData, finalCallback) {
    if (debugBypassConsent) {
      ...
      switch (cmpApi) {
        case 'static':
          this.parseStaticConsentData(providedConsentData);
          finalCallback(this.consentData);
          break;
        case 'iab':
   ...
   */
  parseStaticConsentData(staticConsentData) {
    if (!isPlainObject(staticConsentData)) {
      logError(`cmpApi: 'static' did not specify consent data.`);
      return;
    }

    this.cmpVersion = (staticConsentData.getConsentData) ? 1 : (staticConsentData.getTCData) ? 2 : 0;
    logInfo(`Using static consent data from config for TCF v${this.cmpVersion}`, staticConsentData);

    if (this.cmpVersion === 2) {
      // remove extra layer in static v2 data object so it matches normal v1 CMP object for processing step
      ConsentManagement.storeTcfData(this, staticConsentData.getTCData);
    } else {
      ConsentManagement.storeTcfData(this, staticConsentData);
    }
  }
  ...
  lookupIabConsent(finalCallback) {
    ...
    const afterCallback = () => {
      if (cmpResponse.getConsentData && cmpResponse.getVendorConsents) {
        ConsentManagement.storeTcfData(_this, cmpResponse);
        finalCallback(_this.consentData);
      }
    };
    ...
    const v2CmpResponseCallback = (tcfData, success) => {
      ...
      if (success) {
        if (tcfData.gdprApplies === false || tcfData.eventStatus === 'tcloaded' || tcfData.eventStatus === 'useractioncomplete') {
          ConsentManagement.storeTcfData(_this, tcfData);
          finalCallback(_this.consentData);
        }
      } else {
   ...
   */
  static storeTcfData(consentThis, consentObject) {
    let isValid, storeFn;
    if (consentThis.cmpVersion === 1) {
      isValid = ConsentManagement.isValidV1ConsentObject;
      storeFn = consentThis.storeV1Data;
    } else if (consentThis.cmpVersion === 2) {
      isValid = ConsentManagement.isValidV2ConsentObject;
      storeFn = consentThis.storeV2Data;
    } else {
    ...
    if (isValid(consentObject)) {
      consentThis.consentData = new ConsentData();
      storeFn.call(consentThis, consentObject);
    } else {
