...
export default class ConsentManagement {
  ...
  /**
   * @param {LocalStorage} localStorageDriver the localStorage object to use
   */
  constructor(localStorageDriver) {
  ...
  isProvisionalLocalStorageAllowed() {
    if (!utils.isPlainObject(this.storedPrivacyData)) {
      this.storedPrivacyData = JSON.parse(this.localStorageDriver.getItemWithExpiration(CONSTANTS.STORAGE_CONFIG.PRIVACY));
    }
  ...
  setStoredPrivacy(privacy) {
    try {
      if (utils.isPlainObject(privacy)) {
        ...
        this.localStorageDriver.setItemWithExpiration(CONSTANTS.STORAGE_CONFIG.PRIVACY,
